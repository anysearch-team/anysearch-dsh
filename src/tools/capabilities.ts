...
export function parseCapabilityDomains(domains: string[]): string[] {
  if (domains.length === 0) throw new Error('domains must contain at least one domain')
  ...
  for (const raw of domains) {
    ...
    if (domain.length === 0) throw new Error('domains must not contain blank values')
...
export function formatDomains(result: AnySearchDomainsResponse): string {
...
export function formatSubDomains(result: AnySearchSubDomainsResponse): string {
  const lines = [result.domains.length === 0
    ...
    output: {
      ...
      render: (_args, value) => [{
        ...
        text: value.kind === 'domains'
          ? formatDomains(value)
          : formatSubDomains(value as unknown as AnySearchSubDomainsResponse),
      }],
    },
